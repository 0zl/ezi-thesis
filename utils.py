...
def load_data():
    try:
        ...
        raise FileNotFoundError(f"Dataset files not found in {DATA_DIR}. Ensure std_age.csv and std_height.csv exist.") from e
...
try:
    ...
    print(f"Warning: Data files not found in {DATA_DIR}. Functions depending on data will fail.")
...
def get_z_scores(gender: str, dob: date, weight: float, height: float, measure_mode: str, visit_date: date = date.today()) -> dict:
    """
