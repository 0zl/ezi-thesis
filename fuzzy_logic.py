...
class MalnutritionFuzzySystem:
    def __init__(self):
        ...
        self.bb_u["kurang"] = fuzz.trimf(
            self.bb_u.universe, [-3.5, -2.5, -1.5]
        )  # Dimodif dikit biar overlap-nya bagus
        ...
        r1 = ctrl.Rule(
            self.bb_u["sangat_kurang"] & self.tb_u["sangat_pendek"],
            self.score["gizi_buruk"],
        )
        r2 = ctrl.Rule(
            self.bb_u["sangat_kurang"] & self.tb_u["pendek"], self.score["gizi_buruk"]
        )
        r3 = ctrl.Rule(
            self.bb_u["sangat_kurang"] & self.tb_u["normal"], self.score["gizi_buruk"]
        )
        r4 = ctrl.Rule(
            self.bb_u["sangat_kurang"] & self.tb_u["tinggi"], self.score["gizi_kurang"]
        )  # Agak jarang terjadi, tapi berat rendah + tinggi banget itu tanda kurang gizi

        # 2. Logika Kurang (Underweight)
        r5 = ctrl.Rule(
            self.bb_u["kurang"] & self.tb_u["sangat_pendek"], self.score["gizi_buruk"]
        )
        r6 = ctrl.Rule(
            self.bb_u["kurang"] & self.tb_u["pendek"], self.score["gizi_kurang"]
        )
        r7 = ctrl.Rule(
            self.bb_u["kurang"] & self.tb_u["normal"], self.score["gizi_kurang"]
        )
        r8 = ctrl.Rule(
            self.bb_u["kurang"] & self.tb_u["tinggi"], self.score["gizi_kurang"]
        )

        # 3. Logika Berat Normal
        r9 = ctrl.Rule(
            self.bb_u["normal"] & self.tb_u["sangat_pendek"], self.score["gizi_kurang"]
        )  # Pendek (Stunted) tapi berat normal? Berarti agak gemuk untuk ukuran tingginya
        r10 = ctrl.Rule(
            self.bb_u["normal"] & self.tb_u["pendek"], self.score["gizi_baik"]
        )
        r11 = ctrl.Rule(
            self.bb_u["normal"] & self.tb_u["normal"], self.score["gizi_baik"]
        )
        r12 = ctrl.Rule(
            self.bb_u["normal"] & self.tb_u["tinggi"], self.score["gizi_baik"]
        )

        # 4. Logika Gendut/Risiko Lebih (Overweight)
        r13 = ctrl.Rule(
            self.bb_u["risiko_lebih"] & self.tb_u["sangat_pendek"],
            self.score["gizi_lebih"],
        )
        r14 = ctrl.Rule(
            self.bb_u["risiko_lebih"] & self.tb_u["pendek"], self.score["gizi_lebih"]
        )
        r15 = ctrl.Rule(
            self.bb_u["risiko_lebih"] & self.tb_u["normal"], self.score["gizi_lebih"]
        )
        r16 = ctrl.Rule(
            self.bb_u["risiko_lebih"] & self.tb_u["tinggi"], self.score["gizi_baik"]
        )  # Tinggi + Berat = Proporsional (Normal)
